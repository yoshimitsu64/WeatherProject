...
const openWeatherSlice = createSlice({
  ...
  reducers: {
    ...
    setCurrentWeather: (state, action) => {
      // eslint-disable-next-line no-param-reassign
      state.currentTemperature = action.payload.current?.temp;
      // eslint-disable-next-line no-param-reassign
      state.currentWeather = action.payload.current?.weather[0]?.main;
    },
    setWeather7Days: (state, action) => {
      // eslint-disable-next-line no-param-reassign
      state.weather7days = action.payload.daily.filter((value, index) => index > 0 && index < 7);
    },
  },
});
